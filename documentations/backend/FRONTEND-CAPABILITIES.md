...
```json
{
  "transaction": {
    ...
    "withdraw_memo": "off<compact-order-memo>",
...
The off-ramp `withdraw_memo` is generated by the backend and must be copied
exactly into the Stellar payment. For UUID order IDs it is a 25-byte memo with
an `off` prefix and compact URL-safe encoding; do not derive or rewrite it
from the order ID in the frontend.
