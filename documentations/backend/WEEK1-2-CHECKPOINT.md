...
- Off-ramp deposit instructions, exact amount and memo matching, Horizon
  payment-history scanning, expiry handling, burn-address retirement, and a
  recorded simulated payout.
...
`ADR-001-xendit-native-xlm.md`, `ADR-003-offramp-retirement-payout.md`, and
`ADR-004-persona-kyc-gate.md`. It uses native XLM and a simulated payout. It
does not claim production IDR movement, Stellar mainnet custody, SEP-10
wallet authentication, or a wallet-facing HTML SEP-24 page.
...
| Off-ramp deposit and retirement | Implemented as a sandbox simulation | Correct the public deposit-account field mapping and capture sell-flow evidence. |
...
4. Fix the normal off-ramp response so `order.stellar_destination.account`
   contains the configured deposit account. The current repository stores that
   value in `StellarSource`, while the public serializer reads
   `StellarDestination`.
5. Complete webhook URL revalidation and SSRF controls, HMAC signing, the
   delivery worker, bounded retries, attempt records, and replay guidance.
6. Complete the missing domain, gateway, Stellar, API, and concurrency tests.
...
idempotency keys across uncertain retries, and display the simulated-payout
disclosure on completed sell orders.
